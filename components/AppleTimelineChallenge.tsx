...
import { APPLE_TIMELINE_DATA } from '../challenges/content';

interface AppleTimelineChallengeProps {
...
const AppleTimelineChallenge: React.FC<AppleTimelineChallengeProps> = ({ onComplete, challengeTitle }) => {
    ...
    useEffect(() => {
        const shuffledItems = shuffle(APPLE_TIMELINE_DATA).map((name, index) => ({
            id: index,
            name,
            feedback: null,
        }));
    ...
    const handleCheckOrder = () => {
        ...
        const newFeedbackItems = items.map((item, index) => {
            if (item.name === APPLE_TIMELINE_DATA[index]) {
    ...
    const getItemClass = (item: Item, index: number) => {
        let classes = 'p-3 bg-gray-800/40 border border-white/10 rounded-lg text-white font-semibold transition-all duration-300';
        if (!isSubmitted) {
    ...
    return (
        <div className="p-8 bg-black/30 backdrop-blur-xl border border-white/10 rounded-2xl shadow-glass text-center">
            <h2 className="text-2xl font-bold text-gray-300 mb-2">{challengeTitle}</h2>
            <p className="text-gray-300 mb-6">Arrange the Apple product releases in chronological order from earliest (top) to most recent (bottom).</p>
            ...
            <div className="mt-8">
                {isComplete ? (
                    ...
                    <button
                        ...
                        className="w-full max-w-sm px-8 py-4 bg-gray-600 text-white font-bold rounded-lg hover:bg-gray-700 disabled:bg-gray-600 disabled:cursor-not-allowed transform hover:scale-105 transition-all duration-300 shadow-lg shadow-gray-600/30"
                    >
                        Check Order
                    </button>
                )}
...
export default AppleTimelineChallenge;
