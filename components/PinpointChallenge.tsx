...
import { PINPOINT_CHALLENGE } from '../challenges/content';

interface PinpointChallengeProps {
    onComplete: (success: boolean) => void;
    challengeTitle: string;
}

const PinpointChallenge: React.FC<PinpointChallengeProps> = ({ onComplete, challengeTitle }) => {
    ...
    return (
        <div className="p-8 bg-black/30 backdrop-blur-xl border border-white/10 rounded-2xl shadow-glass text-center">
            ...
            <p className="text-gray-300 mb-6 text-lg">{PINPOINT_CHALLENGE.question}</p>

            <div className="mb-8">
                <a 
                    href={PINPOINT_CHALLENGE.link} 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition-all duration-300 shadow-lg hover:scale-105"
                >
                    Open Pinpoint
...
export default PinpointChallenge;