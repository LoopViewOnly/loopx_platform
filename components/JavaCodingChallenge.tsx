...
const JsCodingChallenge: React.FC<JsCodingChallengeProps> = ({ onComplete, challengeTitle }) => {
    ...
    const validateCode = () => {
        ...
        if (/let\s+[a-zA-Z_$][a-zA-Z0-9_$]*\s*=\s*1\s*;/.test(cleanCode) || /let\s+[a-zA-Z_$][a-zA-Z0-9_$]*\s*=\s*0\s*;/.test(cleanCode)) {
        ...
        // We use \\1 to ensure the variable name matches the one defined in 'let'
        const solution1_regex = new RegExp(
            `for\\s*\\(\\s*let\\s+${id}\\s*=\\s*10\\s*;\\s*\\1\\s*<=\\s*100\\s*;\\s*\\1\\s*\\+=\\s*2\\s*\\)\\s*\\{\\s*console\\.log\\(\\s*\\1\\s*\\)\\s*;?\\s*\\}`,
            's'
        );
        ...
        const solution2_regex = new RegExp(
            `for\\s*\\(\\s*let\\s+${id}\\s*=\\s*10\\s*;\\s*\\1\\s*<=\\s*100\\s*;\\s*\\1\\s*\\+\\+\\s*\\)\\s*\\{\\s*if\\s*\\(\\s*\\1\\s*%\\s*2\\s*===?\\s*0\\s*\\)\\s*\\{\\s*console\\.log\\(\\s*\\1\\s*\\)\\s*;?\\s*\\}\\s*\\}`,
            's'
        );
