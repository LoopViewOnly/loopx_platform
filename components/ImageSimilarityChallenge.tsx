...
const IMAGES = [
  {
    src: BigBean,
    description: "A big bean character sitting happily",
    requiredWords: [
      "mr",
      "bean",
      "big",
  ...
  {
    src: DogWPizza,
    description: "A dog holding a pizza slice in its mouth",
    requiredWords: ["dog", "eating", "pizza", "italy", "grass", "sitting", "pisa" , "tower"],
    minWordsNeeded: 3,
  },
  {
    src: RobotTeaParty,
    description: "A robot having a tea party with toy animals",
    requiredWords: [
      "robots",
      "sitting",
      "moon",
  ...
  {
    src: Sponge,
    description: "A yellow sponge with eyes and a smile",
    requiredWords: [
      "spongebob",
      "riding",
      "unicycle",
  ...
  {
    src: VegiParty,
    description: "Vegetables having a fun party together",
    requiredWords: [
      "banana",
      "wearing",
      "toxido",
...
const ImageSimilarityChallenge = () => {
  ...
  const loadRandomImage = () => {
    const validImages = IMAGES.filter(
      (img) => typeof img.src === "string" && img.src !== ""
    );

    if (validImages.length === 0) {
      console.error(
        "❌ All image imports failed. Check paths or Vite/CRA config."
      );
      return;
    }
  ...
  const handleScorePrompt = () => {
    ...
    setIsLoading(true);
    setStatusMessage("Checking your description against required words...");
    setScore(0);
    setIsSuccess(null);

    setTimeout(() => {
      // Get user's words (case-insensitive)
      const userWords = getTokens(prompt);
      const userWordsSet = new Set(userWords);

      // Get required words for this challenge
      const requiredWords = challenge.requiredWords || [];
      const minWordsNeeded =
        challenge.minWordsNeeded || Math.ceil(requiredWords.length * 0.4);

      // Count how many required words the user used
      let matches = 0;
      const matchedWords = [];
      requiredWords.forEach((word) => {
        if (userWordsSet.has(word.toLowerCase())) {
          matches++;
          matchedWords.push(word);
        }
      });

      // Calculate score as percentage of required words found
      const calculatedScore =
        requiredWords.length > 0
          ? Math.round((matches / requiredWords.length) * 100)
          : 0;

      setScore(calculatedScore);

      // Check if user reaches the threshold line (80%)
      if (calculatedScore >= SIMILARITY_THRESHOLD) {
        setStatusMessage(
          `Success! You used ${matches} out of ${requiredWords.length} required words. Score: ${calculatedScore}%`
        );
        setIsSuccess(true);
      } else {
        setStatusMessage(
          `You used ${matches} out of ${requiredWords.length} required words. You need ${SIMILARITY_THRESHOLD}% to pass. Current score: ${calculatedScore}%`
        );
        setIsSuccess(false);
        // Don't show the target description - it's a cheat!
      }
    ...
    <div className="min-h-screen p-8">
      <div className="max-w-4xl mx-auto p-8 bg-black/30 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl">
        ...
        <p className="text-gray-300 mb-6 text-center">
          Look at the image below. Describe it in detail. Your words will be
          compared to the real image description.
        </p>
        ...
        {/* ⭐ Score bar (unchanged) */}
        <div className="my-6">
          <h3 className="text-center text-lg font-bold text-gray-200 mb-3">
            Similarity Score
          </h3>
          <div className="w-full max-w-2xl mx-auto bg-black/40 rounded-full h-8 shadow-inner relative overflow-hidden border border-white/10">
            ...
            <div
              ...
              style={{ width: `${score}%` }}
            ></div>
            <span className="absolute inset-0 flex items-center justify-center text-white font-extrabold text-sm z-10">
              {score > 0 ? `${score}%` : ""}
            </span>
          </div>
        </div>
          ...
          {statusMessage && (
            <p
              className={`font-medium text-lg ${
                isSuccess === false ? "text-red-400" : "text-green-400"
              }`}
            >
              {statusMessage}
            </p>
